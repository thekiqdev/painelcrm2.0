-- Snapshot de preço contratado (Etapa 1 — só estrutura + backfill; motor de renovação inalterado).
-- pricing_snapshot_source: backfill_last_paid_billing | backfill_current_plan_interval_price |
--   backfill_subscription_amount | backfill_current_plan
...
-- ---------------------------------------------------------------------------
-- Backfill 1: última fatura paga de plano ligada à assinatura (subscription_id)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Backfill 2: última fatura paga por tenant (sem subscription_id ou anterior não cobriu)
-- ---------------------------------------------------------------------------
UPDATE public.subscriptions s
SET
  contracted_at = COALESCE(b.paid_at, b.updated_at),
...
-- ---------------------------------------------------------------------------
-- Backfill 3: valores na própria assinatura (sem fatura paga encontrada)
-- ---------------------------------------------------------------------------
UPDATE public.subscriptions s
SET
...
-- ---------------------------------------------------------------------------
-- Backfill 4a: catálogo atual — standard
-- ---------------------------------------------------------------------------
UPDATE public.subscriptions s
SET
...
-- ---------------------------------------------------------------------------
-- Backfill 4b: catálogo atual — custom (plan_interval_prices)
-- ---------------------------------------------------------------------------
UPDATE public.subscriptions s
SET
