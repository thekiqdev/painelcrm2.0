-- P0 Sprint 2: transactional outbox backbone (shadow / passive-first)
...
COMMENT ON TABLE public.outbox_events IS 'P0 transactional outbox — shadow/passive-first';
COMMENT ON TABLE public.outbox_dispatch_log IS 'Auditoria de tentativas do publisher';
COMMENT ON TABLE public.outbox_subscriber_idempotency IS 'Dedupe de consumo passivo/ativo por subscriber';
COMMENT ON TABLE public.outbox_replay_markers IS 'Foundation replay — liga evento original a re-publicação';

-- Sprint 2 flags (todas OFF por defeito)
