-- Inclui contract_id no payload da função pública de visualização (signatários e PDF público).
-- PostgreSQL não permite mudar o RECORD de retorno com CREATE OR REPLACE: remover versão anterior.
...
COMMENT ON FUNCTION public.get_contract_public_view_by_token_hash(TEXT) IS
  'Read-only público: permite DRAFT; bloqueia cancelado; expõe contract_id para signatários/PDF público.';
