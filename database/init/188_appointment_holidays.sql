-- Agenda Fase 5.4: feriados e bloqueio automático na disponibilidade pública
...
COMMENT ON COLUMN public.appointment_availability_settings.block_holidays IS 'Se true, não gera slots em dias de feriado ativo com blocks_availability.';
COMMENT ON COLUMN public.appointment_availability_settings.holiday_country_code IS 'Filtro de feriados globais (ex.: BR).';
...
CREATE INDEX IF NOT EXISTS idx_appointment_holidays_scope_country ON public.appointment_holidays (scope, country_code)
...
COMMENT ON TABLE public.appointment_holidays IS 'Feriados globais (seed) ou por tenant; recorrentes usam ano na holiday_date só como MD de referência quando is_recurring_yearly.';

-- Seed feriados nacionais fixos BR (ano referência 2000; recorrentes anuais)
