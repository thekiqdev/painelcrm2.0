...
export async function userHasFeature(userId: string, featureKey: string): Promise<boolean> {
  ...
  const tenantRow = await pool.query(
    'SELECT plan_id, status, trial_ends_at FROM tenants WHERE id = $1',
    [tenantId]
  );
  if (tenantRow.rows.length === 0) return false;
  const tenant = tenantRow.rows[0];
  if (tenant.status === 'suspended') return false;
  if (tenant.status === 'trial' && tenant.trial_ends_at && new Date(tenant.trial_ends_at) < new Date()) return false;
...
export async function getEnabledFeaturesForUser(userId: string): Promise<string[]> {
  ...
  if (tenant.status === 'trial' && tenant.trial_ends_at && new Date(tenant.trial_ends_at) < new Date()) return [];
