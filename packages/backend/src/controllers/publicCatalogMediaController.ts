...
/** Contenção de path robusta no Windows (evita falsos negativos com `startsWith` + separadores). */
function isResolvedFileInsideRoot(rootDir: string, absoluteFile: string): boolean {
...
export async function getPublicCatalogMediaRaw(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!isResolvedFileInsideRoot(root, resolvedFile)) {
    ...
    console.error('[publicCatalogMedia] erro inesperado', e);
    if (!res.headersSent) {
      res.status(500).type('text/plain').send('Erro ao servir ficheiro.');
    }
  }
}
