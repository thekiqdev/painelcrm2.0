...
export function OperationTeamSection({ usersCount, maxUsers, onChange }: Props) {
  ...
  return (
    <section className="space-y-2 lg:space-y-3" aria-labelledby="op-team-heading">
      ...
      <div
        className={cn(
          'flex min-w-0 items-center justify-between gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03]',
          'px-2.5 py-2.5 sm:gap-3 sm:px-3 lg:px-3.5 lg:py-2.5',
        )}
      >
        <button
          type="button"
          ...
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-white/10 text-muted-foreground transition-all active:scale-95 hover:border-primary/30 hover:bg-primary/5 disabled:opacity-35 sm:h-11 sm:w-11 lg:h-10 lg:w-10"
        >
      ...
      <ul className="hidden gap-1 lg:grid lg:grid-cols-1 xl:grid-cols-2">
