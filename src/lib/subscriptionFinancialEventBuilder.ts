...
import {
  listCyclesFromDetail,
  timelineRowForCycle,
  type SubscriptionCycleRaw,
} from './subscriptionCyclesSource';
...
function emitEventsForCycleRow(
  ...
  const due = normalizeYmdInput(row.due_date) ?? normalizeYmdInput(cycle.cycle_date);
...
/**
 * Única fonte de eventos financeiros — exclusivamente subscription_cycles (cycles_raw).
 */
export function buildFinancialEvents(
  ...
  for (const cycle of listCyclesFromDetail(normalized)) {
    ...
    if (!row) continue;
    emitEventsForCycleRow(events, seen, normalized, today, cycle, row);
  }
